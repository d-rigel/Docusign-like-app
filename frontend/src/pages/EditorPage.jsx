...
function AttachmentBar({ file, onImport, importing, importStep, canEdit, alreadyImported, wasScanned }) {
  ...
  return (
    <Box sx={{ borderBottom: '1px solid #e5e7eb', bgcolor: '#f8f7f4' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, px: 2.5, py: 1.2, flexWrap: 'wrap', rowGap: 1 }}>
            ...
            <Tooltip title={
              alreadyImported
                ...
                : fileType === 'image'
                  ...
                  : 'Extract content from this file and load it into the editor'
            }>
              <Button
                size="small"
                variant="contained"
      ...
      {importing && (
        <Box sx={{ px: 2.5, pb: 1.5 }}>
          <LinearProgress variant="indeterminate" sx={{ height: 3, borderRadius: 2, mb: 0.5 }} />
          <Typography variant="caption" color="text.secondary">
...
export default function EditorPage() {
  ...
  useEffect(() => {
    ...
    fetchDocument(id)
      .then((doc) => {
        if (doc?.content && doc.content.length > 10) setAlreadyImported(true);
        setDocReady(true);
      })
  ...
  const handleImportFile = useCallback(async () => {
    ...
    setImporting(true);
    setImportStep(0);
    const t1 = setTimeout(() => setImportStep(1), 1500);
    const t2 = setTimeout(() => setImportStep(2), 4000);
    try {
      ...
      if (editorRef.current?.setHtmlContent) editorRef.current.setHtmlContent(html);
      ...
      clearTimeout(t1);
      clearTimeout(t2);
      toast.error(err.response?.data?.error?.message || err.message || 'Could not extract file content', { duration: 6000 });
    } finally {
      setImporting(false);
      setImportStep(0);
    }
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      {/* ── AppBar ── */}
      <AppBar position="static" elevation={0} sx={{ bgcolor: 'primary.main', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
      ...
      {/* ── Body: scroll column + side drawer ── */}
      <Box sx={{ display: 'flex', flexGrow: 1, overflow: 'hidden' }}>
        ...
        <Box sx={{
          ...
          <Box sx={{
            ...
            {/* Editor area with signature overlay */}
