...
export default factories.createCoreController('api::collaborator.collaborator', ({ strapi }) => ({
  ...
  async find(ctx) {
    ...
    if (!documentId) return ctx.badRequest('documentId query param required');
  ...
  async update(ctx) {
    ...
    if ((collab as any).document?.owner?.id !== user.id) {
      return ctx.forbidden('Only the owner can change roles');
    }
